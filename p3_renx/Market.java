...
public class Market extends Space{
    ...
    @Override
    public void greetHeroTeam(HeroTeam heroTeam){
        //0. print Hero Team info, choose a hero in team to trade
        ...
        while(true){
            System.out.println("-------------------------------\n" +Main.ANSI_GREEN+
                    "Welcome to the market! Choose the hero who wants to trade at market, or enter -1 to leave market:"+Main.ANSI_RESET);
            System.out.println(heroTeam);
            input = scanner.nextInt();
            if (input >= 0 && input <= heroTeam.getParty().size()) {
                trade(heroTeam.getParty().get(input));
            } else if (input == -1) {
                System.out.println("-------------------------------\n" +
                        "Your party left the market");
                return;
            } else {
...
//            System.out.println("Hero Inventory: " + hero.getInventory().toString());
            ...
            if (choice == 1) {
                ...
                if (input == 1) {
                    ...
                    if (input >= 0 && input <= marketInventory.gameItemList.size() && marketInventory.gameItemList.get(input).getClass()==Weapon.class) {
                ...
                //armor
                    ...
                    input = scanner.nextInt();
                    //check illegal inputs
                    if (input >= 0 && input <= marketInventory.gameItemList.size() && marketInventory.gameItemList.get(input).getClass()==Armor.class) {
                        if (sellItemToHero(input, hero)) {
                            System.out.println(Main.ANSI_YELLOW+"Purchase successful."+Main.ANSI_RESET);
                        } else {
                            System.out.println("Purchase failed. Check conditions and try again.");
                ...
                //Potion
                    ...
                    input = scanner.nextInt();
                    //check illegal inputs
                    if (input >= 0 && input <= marketInventory.gameItemList.size() && marketInventory.gameItemList.get(input).getClass()==Potion.class) {
                        if (sellItemToHero(input, hero)) {
                            System.out.println(Main.ANSI_YELLOW+"Purchase successful."+Main.ANSI_RESET);
                        } else {
                            System.out.println("Purchase failed. Check conditions and try again.");
                ...
                //Spells
                    ...
                    input = scanner.nextInt();
                    //check illegal inputs
                    if (input >= 0 && input <= marketInventory.gameItemList.size() && marketInventory.gameItemList.get(input) instanceof Spell) {
                        if (sellItemToHero(input, hero)) {
                            System.out.println(Main.ANSI_YELLOW+"Purchase successful."+Main.ANSI_RESET);
                        } else {
                            System.out.println("Purchase failed. Check conditions and try again.");
    ...
    public boolean sellItemToHero(int itemID, Hero buyer){
        ...
        if (buyer.getMoney() < marketInventory.gameItemList.get(itemID).getCost()){
            System.out.println("Not enough Money!");
            return false;
        }
        else {
            buyer.setMoney(buyer.getMoney() - marketInventory.gameItemList.get(itemID).getCost());
            buyer.addToInventory(marketInventory.gameItemList.get(itemID));
            marketInventory.gameItemList.remove(itemID);
            return true;
        }
    }

    public void buyItemFromHero(int itemID, Hero seller){
        GameItem received = seller.removeFromInventory(itemID);
        seller.setMoney(seller.getMoney() + received.getCost());
        marketInventory.gameItemList.add(received);
    }
}
